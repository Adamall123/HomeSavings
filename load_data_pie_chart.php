 <?php
	...
	$numberOfDaysOfSelectedMonth = cal_days_in_month(CAL_GREGORIAN, $month, $year);
	 ...
	 {    
		  ...
		  { 
				$currentDay = date('d');
			   if($stmt = $db->query("SELECT name, SUM(amount)  FROM incomes, incomes_category_assigned_to_users WHERE incomes_category_assigned_to_users.id = incomes.income_category_assigned_to_user_id AND incomes.user_id = '$user_loggedin_id' AND date_of_income >= '$year-01-01' AND date_of_income <= '$year-$month-$currentDay' GROUP BY name")){
				$php_data_array = Array(); // create PHP array
				while ($row = $stmt->fetch()) {
				   $php_data_array[] = $row; // Adding to array
				   }
		  ...
		  {  
			   if($stmt = $db->query("SELECT name, SUM(amount)  FROM incomes, incomes_category_assigned_to_users WHERE incomes_category_assigned_to_users.id = incomes.income_category_assigned_to_user_id AND incomes.user_id = '$user_loggedin_id' GROUP BY name")){
				$php_data_array = Array(); // create PHP array
				while ($row = $stmt->fetch()) {
				   $php_data_array[] = $row; // Adding to array
				   }
